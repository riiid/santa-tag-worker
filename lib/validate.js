...
export const shouldContainSingleCoreTag = tagInfo => {
  let {coreTag, wrong_answers} = tagInfo;
  let result = _.chain(wrong_answers)
    .pluck('wrong_answer_tags')
    .map(wtags => _.pluck(wtags, 'tag'))
    .any(wtags => wtags.length === 1 && wtags[0].name === coreTag.name)
    .value();
  if (!result) {
    reportError(tagInfo, 'cannot pass shouldContainSingleCoreTag');
  }
  return result;
};
