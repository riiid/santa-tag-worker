...
export const report2console = (data, onTest) => {
  let {result, errored} = data;
  let saved = _.filter(result, item => Boolean(item));
  if (!onTest) {
...
export const report2gist = (data, onTest) => {
  let result = {
  ...
  if (onTest) {
    return Rx.Observable.return({result, ids});
  }
  return createGist({
